...
const instructionMessage: ChatCompletionMessageParam = {
  ...
  content: `
    ...
    You should return the translated text in json format. If I don't specify otherwise, you should keep the keys in its original language.
    ...
    The format of the response should be:
    {
      "oldLanguage": "en",
      "newLanguage": "es",
      "translation": "*translated text in json*"
    }
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  const { oldLanguage, newLanguage, translation } = await req.json();
  ...
  const translatedData = JSON.parse(responseMessage);
